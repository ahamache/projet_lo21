...
class Automate2D : public Automate{
    unsigned short int nbMinVivant; /*sachant une cellule vivante, cb il faut de cellule vivante minimum pour qu'elle vive à t+1*/
    unsigned short int nbMaxVivant;
    unsigned short int nbMinMort; /*même choix pour une cellule morte*/
...
class AutomateEpidemie : public Automate {
    ...
    unsigned short int chance1; //étant malade, chance /10 de mourir
	unsigned short int chance2; //étant sain, chance /10 de tomber malade	
    friend class AutomateManager;
...
class AutomateManager{ //Le manager g?re un ensemble d'automate qui peut ?tre 1D ou 2D
    ...
    //Trois tableaux o? on stocke les différents automates :
    ...
    unsigned int nb2DStockes; //le rang du dernier automate 2D rentré
