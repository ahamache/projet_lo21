...
Etat::Etat(unsigned int c, unsigned int l): nbColonne(c), nbLigne(l){
...
Simulateur::Simulateur(const Automate& a, const Etat& dep, unsigned int buffer) : automate(a), etats(nullptr), nbMaxEtats(buffer){
    etats = new Etat*[nbMaxEtats];
	for (unsigned int i = 0; i < nbMaxEtats; i++) etats[i] = nullptr;
	setEtatDepart(dep);
}
